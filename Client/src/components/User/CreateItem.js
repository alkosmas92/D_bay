...
const CreateItem = () => {
  ...
  const [day_end, setDay_end] = useState(d.getDate());
  const [month, setMonth] = useState(d.getMonth());
  const [month_end, setMonth_end] = useState(d.getMonth());
  ...
  for (let i = 0 ; i < isChecked1.length ; i++){  //push id of category base on checked of checkbox
   if(isChecked1[i]===true){
    ...
    const result = await fetch(
      `http://localhost:3000/auth/seller/${userid}/createItem`,
      {
        method: "POST",
        headers: {
    ...
    for (let i=0 ; i < postCategory.length ; i++){
      ...
      let new_category ={
        ...
        itemid: resultInJson.itemid,
      }

      const result = await fetch(
          `http://localhost:3000/auth/seller/${userid}/createCategoryItem`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
